...
- Mermaid diagrams in the Markdown rendered view use a **zero-dependency lite subset** (the full mermaid runtime is ~1MB+, conflicting with the bundle budget): flowchart/graph (TD/LR, node shapes, edge labels, chains, subgraphs) and sequenceDiagram (participants, message arrows, Note over) are supported; other diagram types and uncovered syntax show a centered "diagram failed to parse" note with a source toggle.
