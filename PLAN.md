...
> 2026-08-18 发布修订：包名定为 **`dsh-git-ui`**（因 npm 上存在同名冲突包，经两次更名后定名），
> 已发布至 npm（0.0.1），支持 `dsh plugin --profile web add dsh-git-ui` 直接安装；
> 工程载体迁移至 GitHub: `Julyves/dsh-git-ui`。
...
的功能面全部保留，工程载体为独立仓库（GitHub: `Julyves/dsh-git-ui`，包名 `dsh-git-ui`，已发布 npm 0.0.1）。
...
- 分发三形态（publish.md）：
  ...
  - **tgz**：`pnpm pack` 后 `add file:...tgz`（**免构建权限，推荐发布形态**；待包名改名后
    产出 `dsh-git-ui-*.tgz`）
  - git：`add github:Julyves/dsh-git-ui#<sha>` → 需自包含 `prepare` 脚本（tsc/esbuild，
...
- **patch 行**：单包双 manifest（`dsh.bundle` + `dsh.client`）时**一行 insert 即可**
  ...
  - insert:
      - id: git-ui
        name: dsh-git-ui  # 必须是包名（loader entry name / client.js URL 的依据）
...
├── cordis.patch.yml      # 单行 insert（id: git-ui / name: dsh-git-ui / config）
