...
describe('gitInfoRemote query endpoint', () => {
  ...
  it('accepts turn-records results and rejects unknown entry states (strict mirror)', () => {
    ...
    const goneParsed = gitQueryResultSchema.parse({
      kind: 'turn-records',
      turns: [{ turn: 1, startAt: 0, endAt: null, hasWork: false, narrative: null, internal: [{ path: 'x', status: 'modified', state: 'gone', firstSeenAt: 1 }], sibling: [], external: [] }],
    })
    ...
    expect(() => gitQueryResultSchema.parse({
      kind: 'turn-records',
      turns: [{ turn: 1, startAt: 0, endAt: null, hasWork: false, narrative: null, internal: [{ path: 'x', status: 'modified', state: 'mystery', firstSeenAt: 1 }], sibling: [], external: [] }],
    })).toThrow()
